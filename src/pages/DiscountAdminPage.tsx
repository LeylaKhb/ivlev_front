...
const DiscountAdminPage: React.FC = () => {
    ...
    function handleForm(event: React.FormEvent) {
        ...
        fetch('http://localhost:8080/api/admin_discount', {
            method: 'POST',
            headers: {
    ...
    return (
        <div>
            ...
            <form className="admin_form" onSubmit={handleForm}>
                ...
                <button type="submit" className="change_password_button" style={{marginBottom: 50}}>Найти людей</button>
            ...
            <div style={{width: '100%'}} className="admin_table">
                <table style={{width: '100%'}} className="admin_table">
                    ...
                    {people?.map((person, index) => (
                        <tbody key={index}>
                        <tr style={{ width: '100%'}}>
                            <button  className="login_button" onClick={() => setPopupTrue(person)} style={{marginTop: 20}}>Изменить</button>
