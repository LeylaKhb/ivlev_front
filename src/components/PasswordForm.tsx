...
const PasswordForm: React.FC<PasswordFormProps> = ({handleInput, error, bottom, passwordText, label}) => {
    ...
    return (
        <div style={{width: '100%'}}>
            ...
            <div className="login_form_label" style={{transform: passwordText !== "" ? 'translate(-20px, -20px) scale(0.8)' : "none",
                bottom: bottom}}>{label}</div>
            <div className="login_form_error" style={{display: error === "" ? "none" : "initial",
                bottom: bottom - 22}}>{error}</div>
            <svg className="password_eye_hidden" focusable="false"
                 style={{display: passwordHidden ? "initial" : "none", bottom: bottom - 3}} onClick={() => {setPasswordHidden(false)}}
            ...
            <svg className="password_eye_shown" focusable="false"
                 style={{display: passwordHidden ? "none" : "initial", bottom: bottom - 3}} onClick={() => {setPasswordHidden(true)}}
