...
const PasswordForm: React.FC<PasswordFormProps> = ({handleInput, error, passwordText, label}) => {
    ...
    return (
        <div style={{width: '100%'}}>
            ...
            <div className="login_form_label" style={{transform: passwordText !== "" ?
                    'translate(-20px, -50px) scale(0.8)' : 'translate(20px, -30px) '
                }}>{label}</div>
            <div className="login_form_error" style={{visibility: error === "" ? "hidden" : "visible",
                transform: 'translate(0px, -10px)'}}>{error}</div>
